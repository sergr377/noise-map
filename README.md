...
Если Overpass и тайлы рельефа доступны у вас только через прокси, переменные
`HTTPS_PROXY`/`HTTP_PROXY` должны быть **в окружении того процесса, который
запускает сервер**: без них падает каждый расчёт, в какую точку ни кликни. При
старте сервер печатает, настроен ли прокси, — это первое, что стоит проверить.
