...
| Точка | Без рельефа | С рельефом |
|---|---|---|
| Москва, Хамовники | 110 с | 178 с |
| Москва, Тверская | 216 с | 365 с |
| Москва, Садовое кольцо | 330 с | 582 с |

Разброс дают плотность застройки и настроение публичного Overpass. Поэтому в
интерфейсе честно написано «3–10 минут», а не одно красивое число.
...
10. **`Number(null)` и `Number('')` равны нулю, а ноль — валидная координата.**
...
11. **Округление до сетки должно быть идемпотентным.** Шаг по долготе зависит от
...
> Образ **не собирался и не проверялся** — на машине разработки нет Docker.
> Первую сборку стоит прогнать локально и убедиться, что `ScriptRunner` стартует
> под JRE 17: локально всё проверялось на JDK 25.
...
| | Замерено |
|---|---|
| Пик памяти JVM | **1836 МБ** |
| Время холодного расчёта | 178–582 с |
| Размер образа | ~600 МБ (из них 148 МБ — дистрибутив NoiseModelling) |

Контейнеру нужно **не меньше 2.5 ГБ памяти**, иначе первый же холодный клик
