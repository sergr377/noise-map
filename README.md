...
**Фронтенд написан, но рендер карты не проверен.** Ключ Яндекс Карт, на котором
он тестировался, возвращает `403 Invalid api key` — одинаково с реферером и без,
то есть отклоняется сам ключ, а не домен. Остальная цепочка проверена из браузера:
прокси, кэш, форма и геометрия ответа. Как только ключ заработает, останется
убедиться, что слой изофон рисуется поверх карты.
...
- [ ] **Проверить рендер карты в браузере** — упирается в ключ API, см. ниже.
