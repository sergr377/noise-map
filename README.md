...
ниже), `s3.amazonaws.com` (тайлы рельефа) и `geocode-maps.yandex.ru` (поиск по
адресу). Сама карта грузится в браузере и идёт мимо этих настроек — ей
достаточно системных.
...
сертификат сам. Последним шагом домен добавляется в ограничения ключа
JavaScript API рядом с `localhost`, не вместо него, — без этого карта отдаёт
`403 Invalid api key` при работающем во всём остальном сайте.
...
**Ключ карт остался в истории git** — внутри собранного бандла, коммит `d1db820`,
удалён в `4da2ffd`. Решение владельца репозитория: **оставить как есть**. Ключ
JS API публичен по природе — он и сейчас виден любому посетителю в исходниках
страницы, — и защищён не секретностью, а ограничением по HTTP Referer. Из этого
следует условие, на котором решение держится: **ограничение по referer должно
быть настроено и оставаться настроенным**. Ключа геокодера в истории нет — он
серверный и в бандл никогда не попадал; проверено поиском по всем веткам.
Перевыпуск ключа остаётся вариантом на случай, если ограничение придётся снять.
