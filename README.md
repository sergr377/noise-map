...
через пару минут, точная заменяет её позже.
...
приёмников, те же здания, но источники берутся в пределах 150 м и без рельефа.**
...
150 м вместо 350 ускоряют распространение почти в десять раз. Ослаблять вместо
...
Расплата честная и названа в интерфейсе прямым текстом: предпросмотр занижает
уровни примерно на децибел, и десятая часть площади попадёт в соседнюю полосу.
Для «сравнить районы» этого достаточно — модель и так считает типовой поток по
классу улицы, — а для экспертизы не хватало и точного расчёта. В кэш
предпросмотр не попадает никогда: там обязан лежать только законченный
результат, и отдаётся он с `Cache-Control: no-store`.

Второй проход стоит времени: +12% к холодному расчёту на Тверской, +22% на
разреженной окраине, где сам расчёт короткий. Платит эту цену только тот, кто
ждёт: предпросмотр включается флагом от сервера, как и кадры, поэтому
`prewarm.mjs` считает без него. Выключается переменной `PREVIEW_SRC_DIST=0`.

Проверено на живой задаче в браузере: предварительная карта появилась на 142-й
секунде, точная заменила её на 450-й.
